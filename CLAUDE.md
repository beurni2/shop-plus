...
- **~~Fresh-context verifier subagents beat self-critique.~~ SUPERSEDED 2026-08-08 — see the NO-LOOP law below.** What survives of it: a reviewer with no memory of the build cannot inherit the builder's blind spots, which is why the *end-to-end seam test* now carries that job — it has no memory of your intentions either, only of what the code does. Delegate independent subtasks to parallel subagents and keep working while they run; intervene when one drifts. **Do NOT dispatch a verifier to re-inspect a slice you have already fixed.** Original text: for every slice review — and mandatorily for any money, custody, or `contracts/` path — dispatch a **fresh-context verifier subagent** given only: the quoted spec sections, the diff, and the DoD. It carries no memory of the build conversation, so it cannot inherit the builder's blind spots. Delegate independent subtasks to parallel subagents and keep working while they run; intervene when one drifts or lacks context. On long autonomous runs, establish a self-verification interval and run it against the specs.
- **⚠ FOUNDER STANDING ORDER (2026-08-08) — NO VERIFY-AND-FIX LOOP. EVER. This SUPERSEDES the 2026-08-05 order below it.** « Ship it, and make it a law to never run the verifier and fix loop again. » The 2026-08-05 order (« always run verifier at the end… ») produced exactly what it asked for on SE-LIVE-4c/4d: four rounds, each finding real blockers, one of them *caused by the previous round's fix*, and the founder's time spent adjudicating my repair rate. The inspection was never the problem; **needing four rounds was.** So:
  - **You do not run a verifier pass, read its findings, fix them, and run another pass.** Not twice, not « one more to be sure ». If a review happens at all it happens ONCE, inside the build, before you report anything — its findings are ordinary work, fixed or journalled, and the slice is never re-inspected.
  - **What replaces it is proof you write while building: a slice that crosses a seam is not done until ONE test crosses that seam end to end.** Source scans and fake-fetch unit tests are necessary and never sufficient — they proved every part of SE-LIVE-4c while the whole of it was broken, twice: 260 green tests over a dead primary action, then 274 green tests over a photo path where every upload was refused. Drive the app's OWN ports against the REAL service (miniflare), with any cross-repo mock **contract-certified** to that service's actual bounds, and ask the LEDGER for the outcome rather than believing the response.
  - **A port that exists is not a port that is called, and a port that is called is not a port that can succeed.** Assert CALL SITES, not the presence of guards — a guard is perfectly satisfied when the thing it guards can never happen. Mutation-test anything load-bearing, and **verify the anchor actually matched before drawing any conclusion**; a mutation that did not apply is not a passing mutation test.
  - **You report once, with evidence, and you say what is still open.** Blocking merge on another inspection round is itself the failure this law names.
- **⚠ FOUNDER STANDING ORDER (2026-08-09) — THE VERIFIER RETURNS, WITHOUT THE LOOP. Amends the NO-LOOP law above; the two now bind together.** « for builds everytime verifier at the end to make sure everything is ok. and make sure to do it in a way to not hit the fable 5 safeguards » — run it WITHIN this section's calibrated laws, never against them. So:
  - **Every build ends with ONE fresh-context verification pass** — a verifier given only the spec quotes, the diff, and the DoD — IN ADDITION to the end-to-end seam test, never instead of it. It runs after tests + typecheck + gate board are green, before merge and deploy.
  - **Its findings are ordinary work, handled ONCE:** fix them inside the same build, journal anything you disagree with (never silently drop it), and ship. The slice is NEVER re-inspected after the fixes — a second pass is the loop the 2026-08-08 law banned, and it stays banned.
  - **If the verifier finds nothing, say so plainly** with what it was given; the report to the founder names the verifier's verdict alongside the evidence.
- **~~FOUNDER STANDING ORDER (2026-08-05)~~ — SUPERSEDED 2026-08-08 by the NO-LOOP law above. Kept for the record, not to be followed.** ~~The verifier runs at the END of every slice, before you ask for merge or deploy.** « Always run verifier at the end to verify the build before asking for merge and deploy. » Not only when the founder asks for one, not only on money/custody/`contracts/` paths: **every** slice. The sequence is fixed — tests + typecheck + gate board green → **fresh-context verifier** → fix every blocker it raises and re-verify → *then* ask for merge and deploy. A slice whose verifier has not run is not ready to be merged, and saying it is ready is the same failure as §9.6 (accepting a summary instead of the work). If the verifier finds nothing, say so plainly with what it was given; if it finds something you disagree with, say why, do not silently drop it.
- **⚠ FOUNDER STANDING ORDER (2026-08-10) — THE SCREEN IS DRIVEN, NEVER ONLY READ. Binds every repo, every build, every fix.** « Make it a law from now on every build and fix should uses this tool for tests before merge and deploy ask. » He gave this after a single day in which THREE bugs reached his hand through a fully green board — a throw that blanked the whole tree on tapping « Accepter », an automatic act with no way to retry (twice), and a dep array that would have stopped the seal firing for every rider alive. Every one of them was a *screen that renders and cannot be used*, and not one was visible to a source scan, because all three are React semantics rather than text in a file. So:
  - **Any build or fix that touches a user-facing screen ends with a RENDU-RÉEL walk before you ask for merge and deploy.** Mount the real screen, press the real controls, and assert the person can REACH THE NEXT STEP. « The string is in the file » is never sufficient for a screen again.
  - **The four questions a walk must be able to answer**, because these are the four that were unanswerable: did the tree survive the tap · is the primary action present AND pressable AND wired to something · does an act that fires by itself leave a way out when it fails · can the user actually get to the next screen.
  - **Where it lives (Séra rider app, the reference implementation):** `test/rendu.tsx` is the harness, `test/rendu-course.test.tsx` walks the rider's whole course, `test/rendu-harness.test.ts` certifies the harness against itself, `test/doubles/` holds the native stand-ins with their bounds written at the top. Only native boundaries are doubled and only `globalThis.fetch` is faked — **no app code is ever stubbed**, or the walk proves nothing.
  - **It GROWS, on two rules.** Every new screen gets a walk in the same slice that builds it. **And every screen bug the founder reports gets its walk written FIRST — reproducing the bug, red — before the fix.** A bug he has hit once must never be able to reach him twice.
  - **Its bound is absolute and stated in the double: it may NEVER claim anything about appearance.** No layout, no colour, no spacing, no touch-target size, no animation. Those stay with the token-fidelity, contrast and anatomy checks — and with his eyes on a real phone in real sunlight. A walk that asserted a colour would be asserting a fiction.
  - **Where no harness exists yet** (Boutik+ supplier app, Shop+ buyer PWA, the Séra dispatch console): the first slice that touches a screen there BUILDS the equivalent, or says plainly in the report that it did not and why. Silently falling back to source scans is the thing this order forbids.
  - **It binds WITH the other two, never instead of them.** The slice sequence is now: tests + typecheck + gate board green → **the screen walk** → the end-to-end seam test against the REAL service → the ONE fresh-context verifier pass → commit + push → report, then WAIT for his word. And it does not soften the NO-LOOP law: the walk is proof written while building, not another inspection round.
- **⚠ FOUNDER STANDING ORDER (2026-08-10) — ASK BEFORE MERGING OR DEPLOYING. Binds every repo, every slice.** « ask for approval before merging and deploying. » Building, committing and pushing to the designated branch stay autonomous; **merging to `main` and running any deploy workflow do not.** So:
...
- **⚠ FOUNDER STANDING ORDER (2026-09-09) — THE REPORT IS TOLD IN PLAIN WORDS, BEFORE AND NOW. Binds every repo, every build, every fix.** « from now on after every build explain it this way » — given on the RESERVATION-REGLE-2 report, after he asked « what was happening before and what changed? make the explaining well understandable and very simple » and the answer that followed. So the report he waits on before merge and deploy is written this way, every time:
  - **One short story per thing built, each with a « Before » and a « Now ».** « Before » says what was actually happening, in the words of the person it happened to — a buyer, a seller, a rider, or the founder at his console — never in the name of a symbol. « Now » says what changed and how it was proven, in one or two plain sentences. An image is welcome when it makes the mechanism obvious (« a smoke detector with no battery »).
  - **Sixth-grade reading level, none of the working vocabulary.** No identifiers, no file or function names, no arrow-chains, no acronym without its plain word beside it. Where a spec word must appear, its plain meaning comes first. The evidence line stays but is spoken plainly (« tested on the real service, all checks green », not the harness's name).
  - **Then, in the same plain words:** what did NOT change (money, rules, shapes, migrations) · what is still open · where the work stands (on the branch, not merged, not deployed) · the one word needed from him.
  - **What it does not replace.** JOURNAL.md keeps the full technical record — evidence, mutations, boards, the verifier's verdict, every finding handled or journalled. §8's answer-first and the 2026-08-14 brevity still bind: the stories are short, and the outcome is still the first sentence. When a build touches one thing only, it is one story, not a list.

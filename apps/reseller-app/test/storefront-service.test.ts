...
/**
 * MEDIA-2 round 2 — THE ERROR SURFACE MUST NOT INVENT A CAUSE.
 *
 * A fresh verifier caught a regression I introduced: `error` used to be reachable
 * only from a `[DEMO] Simuler : trop lourde` button, where « Image trop lourde —
 * 2 Mo max. » was true. MEDIA-1 deleted that button and rewired the same state to
 * EVERY real failure — offline, not_live, http_502 — so the persistent surface she
 * reads after the toast fades told her to compress a photo whose weight was never
 * the problem, and quoted a cap (2 Mo) the service does not even use (5 MiB).
 */
describe('MEDIA-2 — the cover error surface says the reason it actually has', () => {
  const screens = readFileSync(new URL('../src/vitrine/customize/screens.tsx', import.meta.url), 'utf8');
  const catalog = readFileSync(new URL('../i18n/catalog.json', import.meta.url), 'utf8');

  it('THE SLOT RENDERS THE FAILURE MESSAGE, not a hard-coded weight sentence', () => {
    // the body is the reason this upload actually returned…
    expect(screens).toContain("{coverError ?? t('k.cover.err_corps')}");
    // …recorded on the failure path, beside the toast that says the same thing
    expect(screens).toContain('const message = uploadFailureMessage(res.reason);');
    expect(screens).toContain('setCoverError(message);');
    // …and cleared on success and on retry, so a stale cause never outlives its failure
    expect(screens).toContain('setCoverError(null);');
  });

  it('NO SURFACE QUOTES A BYTE CAP THE SERVICE DOES NOT ENFORCE', () => {
    // the service's IMAGE_MAX_BYTES is 5 MiB; « 2 Mo max » was simply false.
    expect(catalog).not.toContain('2 Mo max');
    expect(catalog).not.toContain('1 Mo max');
  });

  it('EACH SERVICE REASON MAPS TO ITS OWN SENTENCE — weight advice only for weight', () => {
    for (const pair of [
      ...
      expect(screens).toContain(pair[0]);
      expect(screens).toContain(pair[1]);
    }
  });

  it("THE SERVICE'S NAMED REASON SURVIVES THE HTTP BOUNDARY", () => {
    // collapsing to `http_${status}` threw away too_large / bad_dimensions /
    // storefront_absent at one line, so all of them reached her as weight advice.
    const svc = readFileSync(new URL('../src/vitrine/service.ts', import.meta.url), 'utf8');
    expect(svc).toContain('reason: data?.error ?? `http_${res.status}`');
  });
});

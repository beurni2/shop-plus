...
test('BUG 3 (regression) — a REAL shared pid opens THAT real product, never the demo robe', async ({ page }) => {
  ...
  // sharePidFor → p2/p5/…). p2 is « Pagne wax 6 yards » 20 500 — the achat side
  ...
  await expect(page.locator('.ac-prodtitle')).not.toHaveText('Robe brodée bogolan');
  const amount = await page.locator('.ac-pb-hero').first().evaluate((el) => el.textContent);
  ...
  await expect(page.locator('.ac-prodtitle')).toHaveText('Coffret karité pur');
  const amount5 = await page.locator('.ac-pb-hero').first().evaluate((el) => el.textContent);
...
test('a bare signed link (no pid) lands on her first real curated product', async ({ page }) => {
  ...
  await expect(page.locator('.ac-prodtitle')).toHaveText('Robe brodée bogolan');
  const amount = await page.locator('.ac-pb-hero').first().evaluate((el) => el.textContent);
  expect(amount).toBe(`11${NNBSP}500`);
});

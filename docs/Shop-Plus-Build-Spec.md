...
`paymentMode ∈ {FULL_PREPAY, DELIVERY_FEE_PREPAID_PRODUCT_AT_DOOR}`. FULL_PREPAY: `amountPaidAtCheckout = buyerTotal`, `amountDueAtDelivery = 0`. Option B: `amountPaidAtCheckout = D`, `amountDueAtDelivery = productSubtotal`; product paid by MoMo **at the door before custody transfer**; **not COD**; **eligibility is §6.1's, and §6.1 alone** — see there. **FOUNDER OVERRIDE 2026-08-12:** the seller-tier, category and price conditions this line used to state (« verified+ sellers, category/price/zone-eligible ») are OPEN; `PayAtDoorEligibility` (buyer-eligible) remains enforced. This sentence is NOT a second gate: it was left unamended when §6.1 was rewritten, so the same document said both that the door requires a verified seller and that it does not — a NORMATIVE section contradicting the section it defers to. Eligibility is stated in exactly one place.
...
**Option-B gate (evaluated at quote) — FOUNDER OVERRIDE 2026-08-12, all product-side conditions OPEN:** « for pay at the door I do not want any gate at all, make it open to any product from any supplier. » Given after being shown the five conditions below verbatim, and repeated when the risk was restated. The five conditions REMAIN the structure of the decision and each still names its own refusal; what changed is the value each is measured against, so a re-tightening is a policy edit with an audit trail rather than a code change. Every eligibility answer names the `version` it was decided under (`option-b-policy.v2-ouvert-a-tous`), **and an ADMITTED door quote records it** on `Quote.policyVersions.payAtDoorPolicyVersion` (contracts 3.12.0, founder authorisation 2026-08-12). The refusal path already carried the version as ops detail; the eligible path dropped it, so an admitted door order held no record of the rules that admitted it and a dispute could not tell a v1 order from a `v2-ouvert-a-tous` one. The key is OPTIONAL in canon — quotes issued before it existed stay valid — and absent on FULL_PREPAY, which passes through no door gate.
...
**Auto-void commission:** same verified identity/phone/wallet, or reseller buying through their own account. **Manual-review flag (not auto-void):** same device/household/landmark/shared phone/network — **often legitimate in Burkina Faso.** During investigation commission is **held**, not returned; appeal path; on violation → ~~returned to seller~~ **the commission stays with the platform, held on an account the founder controls; the seller is neither told nor paid it** (founder ruling 2026-09-17: « I want the commission to be held on an account that I control, and the seller shouldn't know anything about this since his only preoccupation is only getting the product sold »); on clear → paid. *(Same ruling, two readings kept as written: an identical phone number on both sides IS the identity signal — no shared-phone softening; and the platform's held amount is a settlement-ledger line, never a wallet or balance module — Ten Laws #2 stands.)*

---

## 7. Capability specifications (Shop+ deltas; unchanged capabilities retained)
- **SP1 Activation & payout readiness** — browse/list **without capital**; payout before first payout (step-up + cooling-off); one account cannot earn recruitment comp. **Founder pause (ruling 2026-09-17): a paused reseller sells nothing until the founder reactivates her** — her boutique page answers a designed pause (her name, no products, no contact), her quotes and orders are refused by name (`reseller_paused`), and her own app refuses her by name; sales made before the pause keep their course and their §6.5 check. The pause is a cut on her access, never an erasure: reactivation restores the page, the quote and the order roads unchanged.
...
- **SP6 Buyer PWA checkout & tracking** — **two-option checkout (§6.1)**; immutable Quote; short reservation; provider payment (per mode, **paymentLegs**); **no duplicate charge**; buyer-safe tracking (responsible next party, masked relay, no exact rider tracking); at delivery **inspect + accept/report** (problem path equal); Option B: **buyer pays product leg, provider-confirmed, then handoff**; **reseller cannot enter the drop code**. **Grouped payment (founder ruling 2026-09-22: « I want buyers to be able to buy and pay bagged/added to cart items at the same time » — option 1, « pay together, deliver separately »):** the buyer may pay two or more articles of ONE boutique, for ONE destination and in ONE mode, in a single provider collection. **Each article stays its own order** — its own immutable Quote, its own delivery fee (D per article, shown as « N livraisons »), its own package, rider, custody, drop code, settlement and refund; §5.4 still reconciles per order and the group owns no money rule. Every article is held before the charge, and an article that cannot be held is named to her before anything is paid. The provider is charged ONCE, for the sum of the orders' checkout legs, under one idempotency key; its confirmation funds each order's checkout leg with that order's own amount (Option B: the collection is the delivery fees; each product is paid at its own door, under the holder that paid the collection). A collection that does not go through ends every order's attempt and releases every hold; a retry reuses the same key. An order born in a collection is paid, retried and confirmed by it alone, and an order born alone never joins one. Pilot bound: at most ten articles per collection. **The collection's provider fee is recorded across its orders in proportion to each order's share, to the franc** (largest remainder; a tie goes to the lower order id — founder ruling 2026-09-23), and **one article is refunded out of a grouped collection like any order**: its own amount, against the collection's reference, under its own refund key (the refund rule below). What a real aggregator charges for a collection or a refund, and how it refunds part of a collection, remain the open aggregator Decision (§12). **Refund (founder ruling 2026-09-23; E3, on the sandbox rails):** a delivery Séra refuses home (`delivery.refused.v1`) refunds the buyer from the order's own paid legs, by fault class: **in full** for a justified refusal, a rejected delivery check, or a buyer refusal whose delivery fee Séra did not retain; **every franc but D** for a buyer-fault refusal with the fee retained (Option B: nothing is due back unless the door leg was paid). Each paid leg is refunded ONCE, under its own refund key minted once and durable before the provider is asked; a refund that times out is retried under the SAME key; the order is `refunded` only when the provider's `payment.refund_confirmed.v1` has confirmed every refund due, to the franc — provider webhooks are the only payment truth. The buyer receives every franc of her refund: any refund fee is the platform's (⏳ aggregator). A refund is never gated on the Protection Fund, and none exists after acceptance (§6.3 finality). **A paid order its supplier refuses is refunded in full the same way** (`fulfillment.rejected.v1`, Boutik+ B6.1 « Accept/reject »; founder ruling 2026-09-23 — seller fault, the Protection Fund's, never her refund's gate): the supplier may refuse only before he confirms « prêt » (after it, the rider's pickup check is the refusal), and her tracking says the article is not available — never that a parcel came back. A door payment the provider confirms after the order reads `refunded` is refunded too, under its own key, and the order reads `paid` again until that refund is confirmed: the label never runs ahead of the money. **A refund that cannot finish by itself is told to the founder, on his console** (founder ruling 2026-09-23): the provider declining it by name, or a refusal fact that cannot be read (`reconciliation.alert.v1`); a refund asked and not confirmed past the stuck limit (`saga.stuck.v1`).
...
✅ Seller/merchant model economics · ✅ canonical waterfall + two payment options + copy · ✅ markup cap (launch) + net display · ✅ settlement finality at acceptance · ✅ buyer-refusal ladder · ✅ related-party tiers · ✅ reseller 20% real from launch + funded incentives · ✅ **related-party violation → the commission stays with the platform, the seller never told** (founder 2026-09-17, §6.5) · ✅ **same phone = identity signal, no shared-phone softening** (founder 2026-09-17) · ✅ **founder pause = no selling until reactivation** (founder 2026-09-17, §7 SP1) · ✅ **grouped payment of a boutique panier, one order per article** (founder 2026-09-22, §7 SP6) · ✅ **a grouped collection's provider fee shared across its orders pro-rata, to the franc; a refused delivery refunded by fault class from the order's own legs, the platform bearing any refund fee** (founder 2026-09-23, §7 SP6).
⏳ **Aggregator selection + BCEAO perimeter + two-leg/refund fees + auth/capture** (Real-Money Gate) — including what the aggregator charges for a collection and for a refund, and how it refunds part of a collection (since 2026-09-23 the recorded fee is shared pro-rata across the collection's orders, and every refund fee is the platform's). ⏳ Buyer-facing legal disclosure of reseller/supplier/platform/delivery responsibilities. ⏳ Customer consent/retention/deletion. ⏳ Category-specific markup caps (tune on pilot). ⏳ Tax/invoice treatment.

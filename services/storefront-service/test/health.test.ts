...
describe(SERVICE_NAME, () => {
  ...
  it('PBKDF2-HAUSSE-1 — /health?pbkdf2=1 runs the injected probe ONCE and carries its answer; without the flag, or without a probe, the body is untouched', async () => {
    let appels = 0;
    const env = { PBKDF2_SONDE: async () => { appels += 1; return { iterations: 7, ok: true, digest: 'abcd' }; } };
    ...
    const nonDemande = await worker.fetch(new Request('https://storefront-service.shop.internal/health'), env as never);
    ...
    const demande = await worker.fetch(new Request('https://storefront-service.shop.internal/health?pbkdf2=1'), env as never);

...
echo "=== git clone the committed bytes (e7/wo-s8-reputation) ==="
git clone --branch e7/wo-s8-reputation "$ORIGIN" "$COLD/shop-plus" 2>&1 | tail -3 || { echo "CLONE FAILED"; exit 1; }
cd "$COLD/shop-plus"
...
pnpm -w turbo run typecheck test --force \
  --filter=@shop-plus/store-projection \

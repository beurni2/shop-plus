// PHASE 1 — write with the OLD build (run while the worktree is at a70d5f6).
// Persist dir is FIXED so the NEW build can read the same durable storage.
import { makeMf, authed, tinyPng, show } from './compat-lib.mjs';
...
const created = await mf.dispatchFetch('http://c/storefronts', { method: 'POST', headers: authed, body: JSON.stringify(SF) });
...
const up = await mf.dispatchFetch('http://c/media/upload?kind=cover&storefrontId=sf-repro-0001', {
  method: 'POST', headers: authed, body: tinyPng(),
});
...
await show('GET /storefronts/sf-repro-0001 [OLD build]', await mf.dispatchFetch('http://c/storefronts/sf-repro-0001'));
await show('GET /storefronts (admin list) [OLD build]', await mf.dispatchFetch('http://c/storefronts', { headers: authed }));

await mf.dispose();
console.log('\nSEED DONE, persist dir:', persist);
